...
async def fetch_test_data(session, nid):
    """
    Fetches test data for a given NID from the API.
...
async def search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if chat_id in ongoing_searches and not ongoing_searches[chat_id].done():
            await safe_send(update.message.reply_text, "⏳ A scan is already running\\. Use `/cancel` to stop it\\.",
                            parse_mode=constants.ParseMode.MARKDOWN_V2)
            return
...
async def perform_search(chat_id, start_nid, end_nid, batch_size, context):
    ...
    try:
        ...
        await safe_send(context.bot.send_message, chat_id=chat_id,
            text=f"✅ Scan finished\\! Checked `{checked_nid_counts.get(chat_id, total)}` NIDs\\.",
            parse_mode=constants.ParseMode.MARKDOWN_V2) # Ensure final count is accurate
